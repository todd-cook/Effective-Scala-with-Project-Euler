...
object problem_30 {
   ...
   * e.g. 1, 2, 3  ->  49, 50, 51
   */
  def stringAsDigitList( str:String): List[Int] = str.toList.map (a =>  java.lang.Integer.parseInt( a +""))

  def numberAsDigitList (num: Int): List[Int] = stringAsDigitList(num.toString)

  def testNumberEqualsPower (num: Int, power:Int): Boolean = numberDigitsToPower(num, power) == num + 0d
